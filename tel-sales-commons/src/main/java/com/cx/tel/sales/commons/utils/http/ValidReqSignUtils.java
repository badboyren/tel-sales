...
public class ValidReqSignUtils {
	 ...
	 * @return
	 */
	public static synchronized void validReqData(HttpServletRequest request,Logger logger ) {
		logger.info("开始验证数据签名信息 ................................");
		if(local.get() == null) {
			...
			while (em.hasMoreElements()) {
				...
				if(local.get().size() > 0) {
					...
					if(StringUtils.isEmpty(reqSign)) {
						logger.info("验证失败：缺少必要业务参数 = {}",param_sign);
						GlobalMsgThrowException.reqParamExp(param_sign);
					}
					if(!sign.equals(reqSign)) {
						logger.info("验证失败： -->请求sign={},服务器={}",reqSign,sign);
						GlobalMsgThrowException.signExp(param_sign);
					}
					logger.info("验证通过：请求参数是：reqdata = {}",JSON.toJSONString(local.get()));
				} else {
					logger.info("没有业务参数,不需要验签..............");
				}
		} else {
